...
x_train = np.random.random((2*n_ranking_train + n_rating_train,1))
uvi_train = np.random.choice(range(2*n_ranking_train), (n_ranking_train,2), replace=False)
uv_train = x_train[uvi_train][:,:,0]

# Get noisy observations f(uv) and corresponding ranks y_train
y_ranking_train, fuv_train = noisy_preference_rank(uv_train,true_sigma)
y_rating_train = obs_function(x_train[2*n_ranking_train+1:,:], true_sigma)
hf,ha = plt.subplots(1,1)
ha.plot(x_plot,y_plot,'r-')
for uv,fuv,y in zip(uv_train, fuv_train, y_ranking_train):
    ha.plot(uv, fuv, 'b-')
...
prefGP = GPpref.PreferenceGaussianProcess(x_train, uvi_train, y_ranking_train, delta_f=delta_f)
