...
wrms_fun = test_data.wrms
wrms_args = {'w_power': 1}

now_time = time.strftime("%Y_%m_%d-%H_%M")
yaml_config ='./config/learned_wine_params.yaml'
...
if 'calc_relative_error' in config['statrun_params']:
...
while trial_number < n_trials:
    ...
    try:
        ...
        rel_obs_fun = GPpref.RelObservationSampler(random_wave.out, config['GP_params']['rel_likelihood'], config['rel_obs_params'])
        abs_obs_fun = GPpref.AbsObservationSampler(random_wave.out, config['GP_params']['abs_likelihood'], config['abs_obs_params'])
